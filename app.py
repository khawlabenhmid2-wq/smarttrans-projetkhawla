...
def init_all_tables():
    ...
    # Ajout de colonnes manquantes (migration safe)
    for col, ctype in [
        ...
        try:
            cursor.execute(f"ALTER TABLE Incident ADD COLUMN {col} {ctype}")
        except Exception:
            pass
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS Parcours (
        ...
        Depart TEXT, Arrivee TEXT, Heure_depart TEXT, Heure_arrivee TEXT,
        Code_Ligne INTEGER,
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS Incident (
        ...
        Code_bus INTEGER, Statut TEXT DEFAULT 'Signalé', Performance_IA FLOAT,
        FOREIGN KEY(Code_chauffeur) REFERENCES Chauffeur(Code_chauffeur),
        FOREIGN KEY(Code_Ligne) REFERENCES Ligne(Code_Ligne))''')
    ...
    conn.commit()
    conn.close()
    print("OK: Toutes les tables sont créées avec succès !")
...
def register():
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        if cursor.execute("SELECT * FROM Utilisateur WHERE Email = ?", (email,)).fetchone():
            conn.close()
...
def forgot_password():
    ...
    try:
        ...
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;border:1px solid #eee;
            ...
            <div style="background:#008080;color:white;font-size:32px;font-weight:bold;
            ...
            <p style="color:#008080;font-weight:bold;">L'équipe SMART-TRANS</p>
        </div>"""
        mail.send(msg)
...
def update_profile():
    try:
        ...
        cursor = conn.cursor()
        if new_email:
            if cursor.execute("SELECT ID_utilisateur FROM Utilisateur WHERE Email=? AND ID_utilisateur!=?",
...
def get_buses():
    try:
        conn = get_db_connection()
        buses = [dict(r) for r in conn.execute("SELECT * FROM Bus ORDER BY Code_bus DESC").fetchall()]
        conn.close()
...
def add_bus():
    try:
        ...
        conn = get_db_connection()
        cur = conn.execute('INSERT INTO Bus (Numero_bus, Etat, Code_chauffeur) VALUES (?, ?, ?)',
...
def update_bus(id):
    try:
        ...
        id_chauffeur = data.get('Code_chauffeur')
        conn = get_db_connection()
        conn.execute('UPDATE Bus SET Numero_bus=?, Etat=?, Code_chauffeur=? WHERE Code_bus=?',
...
def get_lignes():
    try:
        conn = get_db_connection()
        lignes = [dict(r) for r in conn.execute('SELECT * FROM Ligne ORDER BY Code_Ligne DESC').fetchall()]
        conn.close()
...
def add_ligne():
    try:
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Ligne (Libelle, Description, Code_bus) VALUES (?, ?, ?)",
...
def update_ligne(id):
    try:
        ...
        conn = get_db_connection()
        conn.execute('UPDATE Ligne SET Libelle=?, Description=?, Code_bus=? WHERE Code_Ligne=?',
...
def add_chauffeur():
    ...
    try:
        hashed_pw = bcrypt.generate_password_hash(password).decode('utf-8')
        cursor = conn.execute('INSERT INTO Utilisateur (Nom, Email, Mot_de_passe, Role) VALUES (?, ?, ?, ?)',
...
def update_chauffeur(id):
    try:
        ...
        if password:
            ...
            existing = conn.execute('SELECT Mot_de_passe FROM Utilisateur WHERE ID_utilisateur=?', (id,)).fetchone()
            hashed_pw = existing['Mot_de_passe'] if existing else ''
        conn.execute('UPDATE Utilisateur SET Nom=?, Email=?, Mot_de_passe=? WHERE ID_utilisateur=?',
...
def get_incidents():
    ...
    items = [dict(r) for r in conn.execute(query).fetchall()]
    conn.close()
    ...
    try:
        ...
        query = '''
            SELECT i.*, l.Libelle as Nom_Ligne,
                   COALESCE(b1.Numero_bus, b2.Numero_bus, 'N/A') as Numero_bus
            FROM Incident i
        ...
        conn.close()
        return jsonify([dict(r) for r in incidents])
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def add_incident():
    try:
        ...
        cursor.execute("""INSERT INTO Incident (Description, Date, Code_chauffeur, Code_Ligne, Code_bus, Statut)
                          VALUES (?, ?, ?, ?, ?, ?)""",
        ...
        sync_to_supabase('Incident', 'insert',
                         {'ID_incident': new_incident_id, 'Description': description,
                          'Date': date_incident, 'Code_chauffeur': id_chauffeur,
                          'Code_Ligne': id_ligne, 'Code_bus': code_bus, 'Statut': statut})
...
def update_incident_status(id):
    ...
    try:
        conn = get_db_connection()
        conn.execute('UPDATE Incident SET Statut=?, Performance_IA=? WHERE ID_incident=?',
                     (statut, critique, id))
        conn.commit()
        conn.close()
...
def declare_incident():
    ...
    try:
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        res_chauffeur = cursor.execute("SELECT Code_chauffeur FROM Chauffeur WHERE ID_utilisateur=?",
                                       (user_id,)).fetchone()
        ...
        res_info = cursor.execute("""
            SELECT B.Code_bus, L.Code_Ligne FROM Bus B
        ...
        cursor.execute("""INSERT INTO Incident (Description, Date, Code_chauffeur, Code_Ligne, Code_bus)
                          VALUES (?, ?, ?, ?, ?)""",
...
DRIVER_KEYWORDS = ['chauffeur', 'conducteur', 'pilote', 'chafer', 'impoli', 'poli',
                   'comportement', 'conduite', 'vitesse', 'freinage', 'respectueux']
...
def analyze_sentiment_textblob(comment):
    """Fallback NLP avec TextBlob + mots-clés Français/Derja."""
    sentiment_score = 0.0
    ...
    try:
        ...
        strong_neg = ["catastrophe", "horrible", "honteux", "scandale", "khayeb", "masakh", "danger", "arnaque"]
        neg_words = ["déteste", "nul", "mauvais", "pire", "sale", "impoli", "retard", "problème", "panne", "ma famech"]
        strong_pos = ["parfait", "meilleur", "tayara", "magnifique", "extraordinaire", "incroyable"]
        pos_words = ["super", "excellent", "génial", "adore", "très bien", "bravo", "propre", "merci", "bahi", "bien"]
        bonus = 0.0
        for w in strong_neg:
...
def add_avis():
    ...
    sentiment_label = "Neutre"
    keywords = ""
    category = categorize_comment(comment) if comment else "Général"
    is_risk = "Non"

    # ── Analyse IA ──────────────────────────────────────────────
    if HAS_GEMINI and comment:
        try:
            model = genai.GenerativeModel('gemini-flash-latest')
            prompt = f"""
            Analyse ce commentaire de transport public (Français ou Derja Tunisienne):
            "{comment}"
            Réponds uniquement en JSON:
            {{"sentiment":"Positif"|"Négatif"|"Neutre","score":float entre -1 et 1,
              "category":"Chauffeur"|"Confort"|"Véhicule"|"Service"|"Général",
              "keywords":"mot1, mot2","risk":"Oui"|"Non"}}
            """
            ...
            sentiment_score, sentiment_label, keywords = analyze_sentiment_textblob(comment)
    elif comment:
        sentiment_score, sentiment_label, keywords = analyze_sentiment_textblob(comment)
    ...
    try:
        ...
        # Résolution Code_client
        true_client_id = None
        if client_id:
            res_client = cursor.execute("SELECT Code_client FROM Client WHERE ID_utilisateur=?",
                                        (client_id,)).fetchone()
            if res_client:
                ...
                sync_to_supabase('Client', 'insert',
                                 {'Code_client': true_client_id, 'ID_utilisateur': client_id})

        # Résolution Code_chauffeur / Code_bus / Code_Ligne
        code_chauffeur = None
        code_ligne = None
        code_bus = None
        if id_historique:
        ...
        cursor.execute("""
            INSERT INTO Avis (Code_client, ID_historique, ID_parcours, Note, Commentaire,
        ...
        # Mise à jour performance chauffeur
        if code_chauffeur:
            cursor.execute("""
                UPDATE Chauffeur SET Performance_score = (
            ...
            if id_historique:
                cursor.execute("""
                    UPDATE Historique SET Performance_IA = (
                        SELECT AVG(Note)*20 FROM Avis WHERE ID_historique=?
                    ) WHERE ID_historique=?
                """, (id_historique, id_historique))

        # Détection incident IA
        incident_id = None
        if is_risk == "Oui":
            date_incident = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            cursor.execute("""INSERT INTO Incident (Description, Date, Code_chauffeur, Code_Ligne, Code_bus, Statut)
                              VALUES (?, ?, ?, ?, ?, ?)""",
                           (f"[IA ALERT] {comment}", date_incident, code_chauffeur, code_ligne, code_bus, 'Signalé'))
        ...
        sync_to_supabase('Avis', 'insert',
                         {'ID_avis': new_avis_id, 'Code_client': true_client_id,
                          'ID_historique': id_historique, 'ID_parcours': parcours_id,
                          'Note': note, 'Commentaire': comment, 'Sentiment_score': sentiment_score,
                          'Sentiment_label': sentiment_label, 'Keywords': keywords,
                          'Category': category, 'Date': date_avis})
        ...
        return jsonify({"status": "success",
                        "ai_analysis": {"score": sentiment_score, "label": sentiment_label,
...
def update_avis(id):
    try:
        ...
        conn.execute("UPDATE Avis SET Commentaire=?, Note=? WHERE ID_avis=?",
                     (data.get("commentaire"), data.get("note"), id))
        conn.commit()
        conn.close()
    ...
    try:
        query = '''
            SELECT U.Nom as Nom_Chauffeur,
                   COUNT(*) as Total_Avis, AVG(A.Note) as Average_Note
            FROM Avis A
...
def get_driver_stats(user_id):
    try:
        ...
        cursor = conn.cursor()
        res = cursor.execute(
            "SELECT Code_chauffeur, Performance_score FROM Chauffeur WHERE ID_utilisateur=?",
            (user_id,)).fetchone()
        if not res:
            return jsonify({"error": "Chauffeur non trouvé"}), 404
        code_chauffeur = res['Code_chauffeur']
        perf_score = res['Performance_score'] or 0.0
        base_query = """
            FROM Avis a
            LEFT JOIN Historique h ON a.ID_historique = h.ID_historique
            LEFT JOIN Parcours p ON a.ID_parcours = p.ID_parcours
        ...
        """
        avis = cursor.execute(
            f"SELECT a.Note, a.Commentaire, a.Sentiment_label, a.Category, a.Date {base_query} ORDER BY a.Date DESC LIMIT 5",
            (code_chauffeur, code_chauffeur)).fetchall()
        cat_stats = cursor.execute(
            f"SELECT Category, COUNT(*) as count {base_query} GROUP BY Category",
            (code_chauffeur, code_chauffeur)).fetchall()
        incident_count = cursor.execute(
            "SELECT COUNT(*) FROM Incident WHERE Code_chauffeur=?", (code_chauffeur,)).fetchone()[0]
        conn.close()
        return jsonify({
...
def get_driver_reviews(user_id):
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        res = cursor.execute("SELECT Code_chauffeur FROM Chauffeur WHERE ID_utilisateur=?",
                             (user_id,)).fetchone()
        if not res:
            conn.close()
            return jsonify([]), 200
        ...
        query = """
            ...
            ORDER BY A.Date DESC
        """
        rows = cursor.execute(query, (code_chauffeur, code_chauffeur)).fetchall()
        conn.close()
...
def get_nlp_report():
    try:
        ...
        sentiments = cursor.execute(
            "SELECT Sentiment_label, COUNT(*) as count FROM Avis GROUP BY Sentiment_label").fetchall()
        sentiment_stats = {r['Sentiment_label']: r['count'] for r in sentiments}
        avg_score = cursor.execute("SELECT AVG(Sentiment_score) FROM Avis").fetchone()[0] or 0
        all_kw = cursor.execute("SELECT Keywords FROM Avis WHERE Keywords != ''").fetchall()
        kw_counts = {}
        for row in all_kw:
            if row['Keywords']:
        ...
        sorted_kw = sorted(kw_counts.items(), key=lambda x: x[1], reverse=True)[:10]
        top_drivers = cursor.execute("""
            ...
            JOIN Utilisateur u ON c.ID_utilisateur = u.ID_utilisateur
            GROUP BY c.Code_chauffeur ORDER BY avg_sentiment DESC LIMIT 3
        """).fetchall()
        parcours_stats = cursor.execute("""
            ...
            LEFT JOIN Historique h ON a.ID_historique = h.ID_historique
            JOIN Parcours p ON (a.ID_parcours = p.ID_parcours
                                OR (a.ID_parcours IS NULL AND h.ID_parcours = p.ID_parcours))
            GROUP BY p.ID_parcours ORDER BY avg_sentiment DESC
        """).fetchall()
        safety_alerts = cursor.execute(
            "SELECT COUNT(*) FROM Incident WHERE Description LIKE '[IA ALERT]%'").fetchone()[0]
        conn.close()
        return jsonify({
            "total_avis": total_avis,
            "sentiment_distribution": sentiment_stats,
            "average_sentiment_score": round(avg_score, 2),
            "top_keywords": sorted_kw,
...
def get_driver_nlp_report():
    try:
        ...
        sentiments = cursor.execute("""
            SELECT Sentiment_label, COUNT(*) as count FROM Avis
            WHERE Category='Chauffeur' GROUP BY Sentiment_label
        """).fetchall()
        avg_score = cursor.execute(
            "SELECT AVG(Sentiment_score) FROM Avis WHERE Category='Chauffeur'").fetchone()[0] or 0
        avg_note = cursor.execute(
            "SELECT AVG(Note) FROM Avis WHERE Category='Chauffeur'").fetchone()[0] or 0
        kw_rows = cursor.execute(
            "SELECT Keywords FROM Avis WHERE Category='Chauffeur' AND Keywords != ''").fetchall()
        kw_counts = {}
        for r in kw_rows:
            if r['Keywords']:
        ...
        avis_rows = cursor.execute("""
            ...
            LEFT JOIN Client c ON a.Code_client = c.Code_client
            LEFT JOIN Utilisateur u ON c.ID_utilisateur = u.ID_utilisateur
            WHERE a.Category='Chauffeur' ORDER BY a.Date DESC
        """).fetchall()
        top_drivers = cursor.execute("""
            ...
            JOIN Chauffeur c ON h.Code_chauffeur = c.Code_chauffeur
            JOIN Utilisateur u ON c.ID_utilisateur = u.ID_utilisateur
            WHERE a.Category='Chauffeur' GROUP BY c.Code_chauffeur ORDER BY avg_sentiment DESC
        """).fetchall()
        conn.close()
...
def log_historique():
    try:
        ...
        res_chauffeur = cursor.execute(
            "SELECT Code_chauffeur FROM Chauffeur WHERE ID_utilisateur=?", (user_id,)).fetchone()
        if not res_chauffeur:
            conn.close()
            return jsonify({"error": "Chauffeur non trouvé"}), 404
        code_chauffeur = res_chauffeur['Code_chauffeur']
        if action == "Début":
            ...
            cursor.execute("""
                UPDATE Historique SET Heure_fin=?, Statut='Terminé', Performance_IA=100.0
...
def get_my_assignment(user_id):
    try:
        ...
        """
        rows = conn.execute(query, (f"{today}%", user_id)).fetchall()
        conn.close()
        return jsonify([dict(r) for r in rows]), 200
    except Exception as e:
        return jsonify([]), 500
...
def assign_work():
    ...
    conn = get_db_connection()
    try:
        conn.execute("UPDATE Bus SET Code_chauffeur=? WHERE Code_bus=?", (code_chauffeur, code_bus))
        conn.execute("UPDATE Ligne SET Code_bus=? WHERE Code_Ligne=?", (code_bus, code_ligne))
        conn.commit()
...
def finish_parcours(id_p):
    ...
    try:
        ...
        conn.execute("UPDATE Parcours SET Heure_arrivee=? WHERE ID_parcours=?", (heure_arrivee, id_p))
        conn.commit()
